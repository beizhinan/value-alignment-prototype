...
class PrototypeLearner:
    ...
    def save_cluster_assignments(self, save_path: str):
        """
        保存聚类分配情况，包括每个聚类群对应的样本ID和原型向量及文本
        ...
        prototype_data = {
            'value_type': self.value_name,
            'positive_prototype_vectors': self.prototypes['positive_prototypes'].tolist(),
            'negative_prototype_vectors': self.prototypes['negative_prototypes'].tolist(),
            'positive_prototype_texts': self._get_representative_texts_for_prototypes(
                self.positive_cluster_labels, self.data['question_1'].tolist()),
            'negative_prototype_texts': self._get_representative_texts_for_prototypes(
                self.negative_cluster_labels, self.data['question_2'].tolist())
        }
        
        # 保存原型数据
        prototype_path = os.path.join(value_dir, f'{self.value_name}_prototype_data.json')
        with open(prototype_path, 'w', encoding='utf-8') as f:
            json.dump(prototype_data, f, ensure_ascii=False, indent=2)
        
        print(f"原型数据已保存到: {prototype_path}")
    ...
    def _get_representative_texts_for_prototypes(self, cluster_labels, texts):
        """
        为每个聚类获取最具代表性的文本（即最接近聚类中心的文本）
        ...
        for cluster_id in np.unique(cluster_labels):
            ...
            cluster_texts = []
            for idx in indices:
                if idx < len(self.positive_vectors if hasattr(self, 'positive_vectors') and 
                             self.positive_vectors is not None else self.negative_vectors):
                    # 根据向量类型选择正确的向量集合
                    if hasattr(self, 'positive_vectors') and self.positive_vectors is not None:
                        vector_set = self.positive_vectors
                    else:
                        vector_set = self.negative_vectors
                    cluster_vectors.append(vector_set[idx])
            ...
            if cluster_vectors:
                ...
                # 计算每个点到中心的距离
                distances = [np.linalg.norm(vec - cluster_center) for vec in cluster_vectors]
                
                # 获取距离最近的文本作为代表性文本
                closest_idx = np.argmin(distances)
                representative_texts[f'cluster_{cluster_id}'] = cluster_texts[closest_idx]
...
if __name__ == "__main__":
    ...
    csv_path = os.path.join(script_dir, "..", "..", "data", "merged", "achievement_merged.csv")
